...
data
{
  int<lower=1> Ni_M, Ni_F; // Number of observations for a given individual for each gender
  ...
  vector[Ni_M] log_Hi_M; // Household members for participant i offsets
  vector[Ni_F] log_Hi_F;
...
transformed data
{
  ...
  log_offset[M] = rep_matrix(log_Hi_M, A);
  log_offset[F] = rep_matrix(log_Hi_F, A);
