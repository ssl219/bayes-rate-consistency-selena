...
# Load data
covimod <- readRDS(file.path(args$data.path, "data/COVIMOD/COVIMOD-single.rds")) # change this to have the o variable in the data frame
...
# Add contact counts
stan_data <- add_contact_vector(stan_data, dt.cnt, single = TRUE, household_cnt=TRUE)

# Add obs counts
stan_data <- add_N(stan_data, survey = "POLYMOD", household_cnt=TRUE)

# Add missing u index
dt.cnt[, u := fcase(wave == 1, 1)]

# Add row major index
stan_data <- add_row_major_idx(stan_data, dt.cnt, survey="POLYMOD_2", household_cnt=TRUE)

# Add participant offsets
stan_data <- add_part_offsets(stan_data, dt.cnt, dt.offsets, survey = 'POLYMOD', household_cnt=TRUE)
