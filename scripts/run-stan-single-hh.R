...
option_list <- list(
  ...
  optparse::make_option("--chains", type = "integer", default = 8,
                        ...
                        dest = 'chains'),
  optparse::make_option("--model", type = "character", default = "hsgp-eq-rd-poisson",
...
# Load data
covimod <- readRDS(file.path(args$data.path, "data/COVIMOD/COVIMOD-single-hh.rds"))
...
fit$save_object(file = file.path(export.path, paste0(args$model.name, "-hh.rds")))
cat(" DONE!\n")
