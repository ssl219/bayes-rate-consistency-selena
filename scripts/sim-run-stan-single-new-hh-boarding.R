...
option_list <- list(
  ...
  optparse::make_option("--chains", type = "integer", default = 1,
                        ...
                        dest = 'chains'),
  optparse::make_option("--model", type = "character", default = "hsgp-eq-cd-new-hh-dropping-all-zeros-poisson",
...
# Load data
covimod.single.new.hh <- readRDS(file.path(args$data.path, "data/simulations/datasets/new-hh-boarding_school/nodivide-data-hh0.rds"))
...
fit$save_object(file = file.path(export.path, paste0(args$model.name, "-sim-boarding-everyone.rds")))
cat(" DONE!\n")
