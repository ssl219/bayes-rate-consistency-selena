...
# 
```{r}
dt.missing <- dt.hh[wave==1 & (is.na(age)| is.na(gender)), ]
dt.missing
```
...
```
```{r}
unique(d_comb_no_dupl[wave==1 & household_size==2], by="new_id")
```
...
# making sure order of factors in alter_age_strata to be ascending instead of decreasing
d_order_test <- d_order_test[, alter_age_strata_idx:=as.numeric(factor(alter_age_strata, levels=covimod_strata_levels))]
d_order_test <- d_order_test[order(age, new_id, alter_age_strata_idx, gender, alter_gender)]
...
```{r}
d.everything[d.everything$wave==1, ]
```
